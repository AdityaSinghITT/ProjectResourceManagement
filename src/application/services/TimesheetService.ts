...
export class TimesheetService {
  ...
  async getEmployeeAllocations(
    ...
    const employee = await this.requireEmployeeByUserId(userId);
  ...
  async listHistory(userId: number): Promise<TimesheetHistoryResult> {
    const employee = await this.requireEmployeeByUserId(userId);
    appLogger.info('Listing employee timesheet history', { employeeId: employee.id });
    return this.timesheetRepository.listHistoryByEmployee(employee.id);
  }

  async getWeekDetail(userId: number, weekStartInput: string): Promise<TimesheetWeekView> {
    const employee = await this.requireEmployeeByUserId(userId);
    const weekStart = this.resolveWeekStart(weekStartInput);
    const timesheet = await this.timesheetRepository.findByEmployeeAndWeek(employee.id, weekStart);
  ...
  async getReminder(userId: number): Promise<TimesheetReminderResult> {
    ...
    const allocations = await this.allocationRepository.listOverlappingViewsForEmployee(
      employee.id,
      priorWeekStart,
      priorWeekEnd,
    );
    ...
    const priorTimesheet = await this.timesheetRepository.findByEmployeeAndWeek(
      employee.id,
      priorWeekStart,
    );
  ...
  async submit(
    ...
    const existing = await this.timesheetRepository.findByEmployeeAndWeek(employee.id, weekStart);
    if (existing) {
      throw new AppError(
        HttpStatus.CONFLICT,
  ...
  async getTeamTimesheets(
    ...
    const hoursByEmployeeProject = new Map<string, number>();
    const statusByEmployee = new Map<number, TimesheetStatus>();

    for (const row of submittedRows) {
      hoursByEmployeeProject.set(`${row.employeeId}:${row.projectId}`, row.hours ?? 0);
      if (row.timesheetStatus) {
        statusByEmployee.set(row.employeeId, row.timesheetStatus);
      }
    }
    ...
    for (const member of teamMembers) {
      ...
      for (const allocation of allocations) {
        ...
        rows.push({
          ...
          hours: hoursByEmployeeProject.get(hoursKey) ?? null,
          timesheetStatus: statusByEmployee.get(member.id) ?? null,
        });
      }
    }

    const submittedEmployeeIds = new Set(
      submittedRows.map((row) => row.employeeId),
    );
    ...
    return {
      ...
      summary: {
        ...
        submittedCount: submittedEmployeeIds.size,
      },
    };
  }
  ...
  async getEmployeeTimesheetDetail(
    ...
    const weekStart = this.resolveWeekStart(weekStartInput);
    const timesheet = await this.timesheetRepository.findByEmployeeAndWeek(employeeId, weekStart);
  ...
  async listRecentActivityLabels(employeeId: number): Promise<string[]> {
  ...
  private async requireEmployeeByUserId(userId: number) {
