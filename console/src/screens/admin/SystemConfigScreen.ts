...
export const SystemConfigScreen: Screen = {
  ...
  async run(context) {
    ...
    try {
      ...
      console.log('2. Change LLM Provider (GEMINI / GROQ)');
      console.log('3. Update Scheduler Interval');
      console.log('4. Update Max Weekly Hours');
      console.log('5. Back\n');
      ...
      switch (choice) {
        case '1': {
          const llmApiKey = await context.prompt.ask('New API Key: ');
          await context.admin.updateSystemConfig({ llmApiKey });
          console.log('\nAPI key updated.');
          break;
        }
        case '2': {
          const provider = await context.prompt.ask('Provider (GEMINI/GROQ): ');
